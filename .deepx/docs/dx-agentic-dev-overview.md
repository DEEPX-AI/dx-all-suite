...
.deepx/ (canonical source)
   ├── agents/      ←┐
   ├── skills/      ←┤
   ├── templates/   ←┤  dx-agentic-gen generate
   ├── fragments/   ←┘
                          ↓
   ┌──────────────────────┼──────────────────────┐
   ↓                      ↓                      ↓
...
The `dx-agentic-gen` package (`.deepx/tools/`) handles the conversion, and the pre-commit hook (`install-hooks.sh` → `pre-commit-hook.sh`) blocks drift.
...
| `swe-process-gates-internal-dev.md` | SWE discipline when developing internal dx-agentic-dev |
...
| **DEEPX domain** | `dx-agentic-brainstorm` / `dx-agentic-tdd` / `dx-agentic-verify` | DEEPX-specific (model registry check, etc.) |
...
12 skills (same 8 swe-* as suite + 3 agentic-* + dx-skill-router + unique `dx-agentic-runtime-validate`)
...
| Domain skill | Core |
|--------------|------|
| `dx-agentic-app-build-python` | Python variants, IFactory 5 methods |
| `dx-agentic-app-build-cpp` | C++ + CMakeLists.txt |
| `dx-agentic-app-build-async` | AsyncRunner frame overlap |
| `dx-agentic-app-model-management` | model_registry.json query/download |
| `dx-agentic-app-validate` | 5-step verification |
...
|--------------|-----------------|
| `dx-agentic-stream-build-pipeline` | Pipeline **construction** (elements + order) |
| `dx-agentic-stream-build-mqtt-kafka` | Pipeline **messaging** (result external transmission) |
| `dx-agentic-stream-model-management` | Model **registry** (`model_list.json`) |
| `dx-agentic-stream-validate` | Verification **system** (validate_app / validate_framework) |
...
> Outside the 4 repos the user specified, but a summary is included because of its core role in dx-agentic-dev cross-project scenarios.
...
2. /dx-agentic-brainstorm    ← Requirements + 2-3 approaches + user approval
3. /dx-swe-writing-plans     ← Structured plan
4. /dx-agentic-tdd           ← Red(criteria) → Green(generate) → Verify(immediate)
5. /dx-agentic-verify        ← Final evidence (execution output, no conscientious assertions)
```
...
dx-agentic-gen check                                    # drift check
