# DEEPX Agentic Development - dx-agentic-dev (Beta) — `.deepx/` 구조 종합 안내

> 작성 기준: dx-all-suite, dx-runtime, dx_app, dx_stream, dx-compiler 5개 repo의 `.deepx/` 디렉토리 정적 분석
> 작성일: 2026-05-12 · 브랜치: `merge2.3.2/dx-agentic-dev`

---

## 0. 한 줄 요약

DEEPX Agentic Development (dx-agentic-dev) 기능은 **`.deepx/`를 canonical source(SoT)** 로 두고, `dx-agentic-dev-gen` 제너레이터가 Claude Code / Copilot / Cursor / OpenCode 4개 플랫폼용 파일을 자동 생성하는 **단일 출처 다중 출력(single-source / multi-target)** 아키텍처입니다. 5개 repo(suite, runtime, dx_app, dx_stream, compiler)는 같은 골격(agents/, skills/, memory/, templates/, instructions/, toolsets/)을 공유하면서 도메인별로 특화되어 있고, **HARD GATE / Skill Router / Session Sentinel / Output Isolation** 4가지 강제 규칙으로 자동 일관성을 보장합니다.

---

## 1. DEEPX Agentic Development (dx-agentic-dev) 기능 개요
...
`dx-agentic-gen` 패키지(`.deepx/tools/`)가 변환을 담당하고, 사전 커밋 훅(`install-hooks.sh` → `pre-commit-hook.sh`)이 drift를 차단합니다.
...
| `swe-process-gates-internal-dev.md` | 내부 dx-agentic-dev 개발 시 SWE 규율 |
...
| **DEEPX 도메인** | `dx-agentic-brainstorm` / `dx-agentic-tdd` / `dx-agentic-verify` | DEEPX 특화 (모델 레지스트리 체크 등) |
...
12개 (suite와 동일한 swe-* 8개 + agentic-* 3개 + dx-skill-router + 고유 `dx-agentic-runtime-validate`)
...
| 도메인 스킬 | 핵심 |
|------------|------|
| `dx-agentic-app-build-python` | Python 변형, IFactory 5-메서드 |
| `dx-agentic-app-build-cpp` | C++ + CMakeLists.txt |
| `dx-agentic-app-build-async` | AsyncRunner 프레임 중첩 |
| `dx-agentic-app-model-management` | model_registry.json 쿼리/다운로드 |
| `dx-agentic-app-validate` | 5단계 검증 |
...
| 도메인 스킬 | 핵심 차이 |
|------------|----------|
| `dx-agentic-stream-build-pipeline` | 파이프라인 **구성** (엘리먼트 + 순서) |
| `dx-agentic-stream-build-mqtt-kafka` | 파이프라인 **메시징** (결과 외부 전송) |
| `dx-agentic-stream-model-management` | 모델 **레지스트리** (`model_list.json`) |
| `dx-agentic-stream-validate` | 검증 **체계** (validate_app / validate_framework) |
...
> 사용자가 명시한 4개 repo 외이지만, dx-agentic-dev cross-project 시나리오에서 핵심 역할이므로 요약 포함.
...
2. /dx-agentic-brainstorm    ← 요구사항 + 2-3 접근법 + 사용자 승인
3. /dx-swe-writing-plans     ← 구조화된 plan
4. /dx-agentic-tdd           ← Red(criteria) → Green(generate) → Verify(즉시)
5. /dx-agentic-verify        ← 최종 evidence (실행 출력, 의식적 단언 금지)
```
