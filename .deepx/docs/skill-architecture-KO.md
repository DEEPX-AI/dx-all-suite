...
| **End-User (Agentic Dev)** | `dx-agentic-*` | dx-agentic-dev 기능을 통해 app/pipeline 빌드 | `dx-agentic-tdd` |
| **Harness Eng** | `dx-harness-*` | 내부 `.deepx/`, `tests/`, `tools/` 유지보수 | `dx-harness-validate` |
| **Meta** | `dx-skill-router` | 모든 tier에서 사용 | — |

### 설계 원칙

1. **`dx-agentic-*`는 `dx-swe-*`를 참조한다** — agentic skill은 DEEPX 전용
...
3. **`dx-harness-*`는 내부 전용이다** — 이 skill들은 end-user task에서 호출되지
   ...
   `dx-agentic-gen`) 를 사용합니다.

4. **Domain 전용 skill은 `dx-agentic-{subproject}-*` 명명 규칙을 따른다** — 각
...
| `dx-agentic-tdd` | dx_app, dx_stream, Integration에 대한 Validation Order | `dx-swe-tdd` |
| `dx-agentic-verify` | dx_app, dx_stream, Cross-Project에 대한 Checklist | `dx-swe-verify` |
...
- **`dx-agentic-*`** sub-project 레벨은 project 전용 세부 사항을 포함합니다
  (예: dx_app의 `dx-agentic-tdd`는 133-model validation 세부 사항을 가짐)
...
| Skill | Purpose |
|-------|---------|
| `dx-agentic-compiler-compile` | ONNX → DXNN compilation 단계별 workflow |
| `dx-agentic-compiler-convert` | PyTorch → ONNX 변환 단계별 workflow |
| `dx-agentic-compiler-validate` | compile된 .dxnn model 출력 validation |

#### dx-runtime Domain Skills

| Skill | Purpose |
|-------|---------|
| `dx-agentic-runtime-validate` | Validate, feedback 수집, fix 적용, verify |

#### dx_app Domain Skills

| Skill | Purpose |
|-------|---------|
| `dx-agentic-app-build-python` | Python inference app 빌드 (IFactory + SyncRunner) |
| `dx-agentic-app-build-cpp` | C++ inference app 빌드 |
| `dx-agentic-app-build-async` | async 고성능 app 빌드 (AsyncRunner) |
| `dx-agentic-app-model-management` | Model 다운로드 및 설정 |
| `dx-agentic-app-validate` | dx_app validation check 실행 |

#### dx_stream Domain Skills

| Skill | Purpose |
|-------|---------|
| `dx-agentic-stream-build-pipeline` | GStreamer pipeline app 빌드 |
| `dx-agentic-stream-build-mqtt-kafka` | MQTT/Kafka pipeline app 빌드 |
| `dx-agentic-stream-model-management` | Model 다운로드 및 설정 |
| `dx-agentic-stream-validate` | dx_stream validation check 실행 |

## 필수 Skill Sequence

### End-User Scenario

발동 조건: task가 `dx-agentic-dev/<session_id>/`에 파일을 생성할 때

```
dx-skill-router
  → dx-agentic-brainstorm    (sub-project로 routing, dx-swe-brainstorm 참조)
  → dx-swe-writing-plans
  → dx-agentic-tdd           (dx-swe-tdd 참조)
  → dx-agentic-{subproject}-* (domain build skill: compile, build-python 등)
  → dx-agentic-verify        (dx-swe-verify 참조)
```
...
`tests/`, `tools/`, `dx-agentic-dev/`는 제외)
...
`dx-agentic-*` skill은 `dx-swe-*`를 직접 참조합니다:
...
getting-started test). `dx-agentic-dev/e2e-tests/`도 현재 위치에 유지됩니다
