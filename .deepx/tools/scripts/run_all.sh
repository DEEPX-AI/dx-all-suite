...
# Resolve a working dx-agentic-gen invocation: prefer the installed CLI, else
# fall back to running the module from the in-tree source (.deepx/tools/src).
# Without this, `python -c "import dx_agentic_dev_gen"` fails when the package
...
gen() {  # gen <subcommand> --repo <path>
  ...
  if PYTHONPATH="$SUITE_ROOT/.deepx/tools/src${PYTHONPATH:+:$PYTHONPATH}" \
       python3 -c "import dx_agentic_dev_gen.cli" 2>/dev/null; then
    PYTHONPATH="$SUITE_ROOT/.deepx/tools/src${PYTHONPATH:+:$PYTHONPATH}" \
      python3 -c "import sys; from dx_agentic_dev_gen.cli import main; sys.exit(main(sys.argv[1:]))" "$@"
  elif command -v dx-agentic-gen &>/dev/null && dx-agentic-gen --help >/dev/null 2>&1; then
    dx-agentic-gen "$@"
  else
    echo "ERROR: dx-agentic-gen is not runnable (no importable .deepx/tools/src and no working CLI)." >&2
    return 2
  fi
}
