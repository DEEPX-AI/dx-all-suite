...
| Showcase | 설명 | 빌드 시간 | Agent turns | Output tokens | ~비용 |
|---|---|---|---|---|---|
| **[PDF → Markdown (문서 변환 앱)](../../dx-agent-dev-showcase/rapiddoc-pdf2md/)** | PDF(디지털/스캔)를 구조화된 Markdown + JSON으로 변환 — 레이아웃 분석·OCR·표·수식 — 을 RapidDoc fork로 DEEPX DX-M1 NPU에서 실행. `--parse-method auto|txt|ocr` 지원. | ≈ 12 min | 61 | ≈ 51.8K | ≈ $6.2 |
