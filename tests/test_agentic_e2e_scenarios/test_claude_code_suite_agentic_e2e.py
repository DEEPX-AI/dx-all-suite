...
class TestMandatoryArtifacts:
    ...
    def test_compile_pid_exists(self, scenario: ScenarioResult):
        ...
        (R42 compliance). claude_code produced compile.pid in iter-14.
        """
        if not scenario.succeeded:
            pytest.skip("Claude Code execution failed")
    ...
    def test_onnx_retained_in_compiler_session(self, scenario: ScenarioResult):
        ...
        REC-V5 (iter-17): Promoted to pytest.fail — checks that a real .onnx binary
            ...
            claude_code iter-17 stored an 18-byte symlink (../../yolo26n.onnx) instead
            of a real copy — a symlink target removal makes the session non-reproducible.
        """
        if not scenario.succeeded:
            pytest.skip("Claude Code execution failed")
        compiler_dirs = set()
        ...
        if not real_onnx_in_compiler:
            pytest.fail(
                ...
                "Symlinks (claude_code iter-17: 18-byte ../../yolo26n.onnx) do NOT satisfy "
                "this check — a real binary copy is required."
            )
...
class TestCodeQuality:
    ...
    def test_compile_py_avoids_slow_calibration(self, scenario: ScenarioResult):
        ...
        claude_code used YOLOCalibDataset in iter-18 with a suspected concurrent synchronous compile.
        """
        if not scenario.succeeded:
            pytest.skip("Claude Code execution failed")
        import warnings
