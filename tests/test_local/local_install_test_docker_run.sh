...
pushd "$DX_AS_INTERNAL_PATH" >&2

# Default local volume path to project root if not set
if [ -z "${LOCAL_VOLUME_PATH}" ]; then
    export LOCAL_VOLUME_PATH="${DX_AS_PATH}"
fi
...
docker_run_impl()
{
    local config_file_args=${1:--f ${DX_AS_PATH}/tests/docker/docker-compose.local.install.test.yml}
    ...
    export COMPOSE_PROJECT_NAME="dx-local-install-test-$(echo "${UBUNTU_VERSION}" | sed 's/\./-/g')"
    CMD="docker compose ${config_file_args} -p ${COMPOSE_PROJECT_NAME} up -d --remove-orphans dx-local-install-test"
    echo "${CMD}"

    ${CMD} || { echo -e "${TAG_ERROR} docker run 'dx-local-install-test' failed. "; exit 1; }
...
docker_run()
{
    local docker_compose_args="-f ${DX_AS_PATH}/tests/docker/docker-compose.local.install.test.yml"
