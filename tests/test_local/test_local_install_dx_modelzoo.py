...
# Test configuration
TEST_TIMEOUT = 7200  # 2 hours for modelzoo install
...
def _ensure_repo_in_container(container_name: str) -> None:
    ...
    install_path = f"{repo_root_path}/dx-modelzoo/install.sh"
    nested_install_path = f"{nested_repo_path}/dx-modelzoo/install.sh"
...
class TestLocalInstallDxModelzoo:
    ...
    def _run_with_live_output(
        ...
        banner = (
            f"\n{'=' * 80}\n"
            f"🚀 Installing dx-modelzoo in container: {container}\n"
            f"{'=' * 80}\n"
        )
        ...
        if process.returncode == 0:
            summary += f"✅ dx-modelzoo install succeeded in {container}\n"
        else:
            summary += f"❌ dx-modelzoo install failed in {container} (exit code: {process.returncode})\n"
        summary += f"{'=' * 80}\n"
