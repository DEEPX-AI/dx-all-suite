...
# Test configuration
TEST_TIMEOUT = 7200  # 2 hours for compiler install
...
def _ensure_repo_in_container(container_name: str) -> None:
    ...
    install_path = f"{repo_root_path}/dx-compiler/install.sh"
    nested_install_path = f"{nested_repo_path}/dx-compiler/install.sh"
...
class TestLocalInstallDxCompiler:
    ...
    def _run_with_live_output(
        ...
        banner = (
            f"\n{'=' * 80}\n"
            f"🚀 Installing dx-compiler in container: {container}\n"
            f"{'=' * 80}\n"
        )
        ...
        if process.returncode == 0:
            summary += f"✅ dx-compiler install succeeded in {container}\n"
        else:
            summary += f"❌ dx-compiler install failed in {container} (exit code: {process.returncode})\n"
        summary += f"{'=' * 80}\n"
