...
응답 언어는 사용자의 프롬프트 언어와 일치시키세요 — 질문하거나 응답할 때,
사용자가 사용하는 언어와 동일한 언어를 사용하세요.

**기술 용어 규칙**: 한국어로 응답할 때, 영어 기술 용어는 원래 영어 형태를 유지하세요.
영어 용어를 한글 음차로 표기하지 마세요 (한글 음차 표기 금지). 정착된 한국어
외래어(모델, 서버, 파일, 데이터)는 허용됩니다.

## 권장 모델 (사전 비행 점검)

DX Agentic Development는 강력한 instruction following과 knowledge base 활용이
필요합니다. 작업을 시작하기 전에, 권장 모델 중 하나로 실행되고 있는지 확인하세요:
...
안내 박스 출력 후, 작업을 정상적으로 진행하세요. 이것은 정보성 경고이며,
차단 게이트가 아닙니다.
...
이 리마인더는 한 번만 출력하세요 (첫 번째 질문 앞), 매 질문마다 출력하지 마세요.
...
| `/dx-tdd` | 테스트 주도 개발 — 증분 검증 |
| `/dx-verify-completion` | 완료 주장 전 검증 — 주장 전 증거 |
...
절대 stub/placeholder 코드를 생성하지 마세요. 여기에는 다음이 포함됩니다:
...
모든 생성된 코드는 knowledge base의 실제 API를 사용하여 기능해야 합니다.
필요한 SDK/API를 모르는 경우, 관련 스킬 문서를 먼저 읽으세요.

## 실험적 기능 — 금지

실험적이거나 존재하지 않는 기능을 제안, 제시, 또는 구현하지 마세요. 여기에는
다음이 포함됩니다:
...
DEEPX 프로젝트에서 반드시 건너뛰어야 합니다. visual companion은 우리 환경에
존재하지 않습니다. 브레인스토밍 체크리스트에서 "Offer visual companion"이라고
하면, 건너뛰고 "Ask clarifying questions" (3단계)로 직접 진행하세요.

기능이 존재하지 않으면, 존재하는 척하지 마세요. 검증된, 문서화된 기능만
사용하세요.

**Autopilot / autonomous mode 오버라이드**: 사용자가 부재한 경우 (autopilot mode,
auto-response "work autonomously", 또는 `--yolo` 플래그), 브레인스토밍 스킬의
"Ask clarifying questions" 단계는 "knowledge base 규칙에 따른 기본 결정 수행"으로
대체되어야 합니다. `ask_user`를 호출하지 마세요 — knowledge base 기본값을
사용하여 브레인스토밍 spec 생성으로 바로 진행하세요. 이후의 모든 게이트
(spec 리뷰, plan, TDD, mandatory artifacts, execution verification)는 예외 없이
적용됩니다.
...
1. **Spec 문서는 MANDATORY** — `writing-plans`로 전환하기 전에, spec 문서가
   `docs/superpowers/specs/YYYY-MM-DD-<topic>-design.md`에 반드시 작성되어야
   합니다. spec을 건너뛰고 바로 plan 작성으로 가는 것은 위반입니다.
2. **사용자 승인 게이트는 MANDATORY** — spec 작성 후, 사용자가 plan 작성으로
   진행하기 전에 반드시 검토하고 승인해야 합니다. 관련 없는 사용자 응답
   (예: 다른 질문에 대한 답변)을 spec 승인으로 취급하지 마세요.
3. **Plan 문서는 spec을 참조해야 함** — plan 헤더에 승인된 spec 문서에 대한
   링크가 포함되어야 합니다.
4. **`/dx-brainstorm-and-plan` 선호** — 일반적인 superpowers `brainstorming`
   스킬 대신 프로젝트 수준의 브레인스토밍 스킬을 사용하세요. 프로젝트 수준
   스킬에는 도메인별 질문과 사전 비행 점검이 있습니다.
5. **규칙 충돌 확인은 MANDATORY** — 브레인스토밍 중, agent는 사용자 요구사항이
   HARD GATE 규칙 (IFactory 패턴, skeleton-first, Output Isolation,
   SyncRunner/AsyncRunner)과 충돌하는지 반드시 확인해야 합니다. 충돌이
   감지되면, agent는 브레인스토밍 중에 해결해야 합니다 — 설계 spec에서
   위반 요청을 조용히 따르지 마세요. 위의 "규칙 충돌 해결"을 참조하세요.

## 자율 모드 보호 (MANDATORY)

사용자가 부재한 경우 — autopilot mode, `--yolo` 플래그, 또는 시스템 자동 응답
"The user is not available to respond" — 다음 규칙이 적용됩니다:

1. **"자율적으로 작업하라"는 "질문 없이 모든 규칙을 따르라"는 의미이지, "규칙을
   건너뛰라"가 아닙니다.** 모든 mandatory 게이트가 여전히 적용됩니다:
   브레인스토밍 spec, plan, TDD, mandatory artifacts, execution verification,
   self-verification 점검.
2. **`ask_user`를 호출하지 마세요** — knowledge base 기본값과 문서화된 모범
   사례를 사용하여 결정하세요. autopilot에서 `ask_user` 호출은 턴을 낭비하며
   자동 응답은 어떤 게이트도 우회할 권한을 부여하지 않습니다.
3. **사용자 승인 게이트 적응** — autopilot에서 spec 승인 게이트는 spec을 작성하고
   knowledge base에 대해 자체 검토하는 것으로 충족됩니다. spec을 완전히 건너뛰지
   마세요.
4. **setup.sh 우선** — 애플리케이션 코드를 작성하기 전에 인프라 아티팩트
   (`setup.sh`, `config.json`)를 생성하세요. 이는 autopilot에서 특히 중요합니다.
   누락된 의존성을 잡을 사람이 없기 때문입니다.
5. **실행 검증은 선택이 아님** — 생성된 코드를 실행하고 완료 선언 전에 작동하는지
   확인하세요. autopilot에서는 오류를 잡을 사용자가 없습니다.
6. **시간 예산 인식** — Autopilot 세션에는 시간 제약이 있을 수 있습니다.
   효율적으로 행동을 계획하세요:
   - 컴파일 (ONNX → DXNN)은 5분 이상 걸릴 수 있습니다 — 일찍 시작하세요.
   - 시간이 부족하면, execution verification보다 아티팩트 생성을 우선시하세요
     — 테스트되지 않은 완전한 파일 세트가 테스트된 불완전한 세트보다 낫습니다.
   - 우선순위 순서: `setup.sh` > `run.sh` > app code > `verify.py` > session.log.
   - **컴파일 병렬 워크플로우 (HARD GATE)** — bash 명령에서 `dxcom` 또는
     `dx_com.compile()`을 시작한 후, 기다리지 마세요. 즉시 모든 mandatory
     artifacts를 생성하세요: factory, app code, setup.sh, run.sh, verify.py.
     다른 모든 아티팩트가 생성된 후에만 `.dxnn` 출력을 확인하세요. **이 규칙을
     위반하면 세션이 실패합니다.**
   - **컴파일을 위한 sleep-poll 절대 금지** — `.dxnn` 파일을 폴링하기 위해
     루프에서 `sleep`을 사용하지 마세요. 금지된 패턴에는 다음이 포함됩니다:
     `for i in ...; do sleep N; ls *.dxnn; done`,
     `while ! ls *.dxnn; do sleep N; done`,
     사이에 대기가 있는 반복적인 `ls *.dxnn` / `test -f *.dxnn` 확인.
     대신: 다른 모든 아티팩트를 먼저 생성한 다음, `.dxnn` 파일이 존재하는지
     한 번만 확인하세요. 아직 존재하지 않으면, 컴파일이 완료될 것이라는
     가정 하에 execution verification으로 진행하세요.
   - **Mandatory artifacts는 컴파일과 독립적** — `setup.sh`, `run.sh`,
     `verify.py`, factory, app code는 `.dxnn` 파일이 존재할 필요가 없습니다.
     알려진 모델 이름 (예: `yolo26n.dxnn`)을 placeholder 경로로 사용하여
     생성하세요. execution verification만 실제 `.dxnn`이 필요합니다.
7. **파일 읽기 도구 호출 최소화** — 이미 컨텍스트에 로드된 instruction 파일,
   agent 문서, 또는 스킬 문서를 다시 읽지 마세요. 불필요한 `cat` / `bash` 읽기는
   각각 5-15초를 낭비합니다. 시스템 프롬프트와 대화 기록에 있는 지식을
   사용하세요.
...
**절대 `docs/superpowers/` 아래의 파일을 `git add`하거나 `git commit`하지 마세요.**
이것들은 superpowers 스킬 시스템 (specs, plans)에 의해 생성된 임시 계획
아티팩트입니다. `.gitignore`에 포함되어 있지만, 일부 도구는 `git add -f`로
`.gitignore`를 우회할 수 있습니다. 파일 생성은 괜찮습니다 — 커밋하는 것은
금지됩니다.

## 세션 센티넬 (자동화 테스트를 위한 MANDATORY)

사용자 프롬프트를 처리할 때, 테스트 하네스에 의한 자동화된 세션 경계 감지를 위해
다음 마커를 정확히 출력하세요:
...
1. **중요 — `[DX-AGENTIC-DEV: START]`를 첫 번째 응답의 절대 첫 줄로 출력하세요.**
   이것은 다른 텍스트, 도구 호출, 또는 추론보다 먼저 나타나야 합니다. 사용자가
   "그냥 진행해" 또는 "알아서 판단해"라고 지시하더라도, START 센티넬은 협상
   불가입니다 — 자동화 테스트가 이것 없이는 실패합니다.
2. 모든 작업, 검증, 파일 생성이 완료된 후 맨 마지막 줄로
   `[DX-AGENTIC-DEV: DONE (output-dir: <path>)]`을 출력하세요
3. 상위 레벨 agent에서 handoff/routing을 통해 호출된 **하위 agent**인 경우,
   이 센티넬을 출력하지 마세요 — 최상위 agent만 출력합니다
4. 사용자가 세션에서 여러 프롬프트를 보내면, 각 프롬프트에 대해 START/DONE을
   출력하세요
5. DONE의 `output-dir`는 프로젝트 루트에서 세션 출력 디렉토리까지의 상대 경로여야
   합니다. 파일이 생성되지 않았으면 `(output-dir: ...)` 부분을 생략하세요.
6. **계획 아티팩트만 생성한 후에는 절대 DONE을 출력하지 마세요** (specs, plans,
   설계 문서). DONE은 모든 산출물이 생성되었음을 의미합니다 — 구현 코드, 스크립트,
   설정, 검증 결과. 브레인스토밍이나 계획 단계를 완료했지만 아직 실제 코드를
   구현하지 않았다면, DONE을 출력하지 마세요. 대신 구현으로 진행하거나 사용자에게
   어떻게 진행할지 물어보세요.
7. **DONE 전 mandatory deliverable 점검**: DONE을 출력하기 전에, 아래의 자체 검증
   점검을 실행하세요. mandatory 파일이 누락된 경우, DONE을 출력하기 전에
   생성하세요. **절대 이 단계를 건너뛰지 마세요.**
   ```bash
   ...
   ```
   아티팩트 중 하나라도 MISSING으로 표시되면, 돌아가서 생성하세요. 누락된
   아티팩트와 함께 최종 보고서를 제시하거나 DONE을 출력하지 마세요.
8. **세션 HTML export 안내** (Copilot CLI에만 해당): DONE 센티넬 줄 바로 앞에
   ...
   또는 OpenCode에서는 작동하지 않습니다. 테스트 하네스 (`test.sh`)는 자동으로
...
사용자의 요청이 HARD GATE 규칙과 충돌하는 경우, agent는 반드시:

1. **사용자의 의도를 인정** — 원하는 것을 이해하고 있음을 보여주세요.
2. **충돌을 설명** — 특정 규칙과 존재 이유를 인용하세요.
3. **올바른 대안을 제안** — 프레임워크 내에서 사용자의 목표를 달성하는 방법을
   보여주세요. 예를 들어, 사용자가 직접 `InferenceEngine.run()` 사용을 요청하면,
   IFactory 패턴이 동일한 API를 래핑하고 있음을 설명하고 factory 기반 동등물을
   제안하세요.
4. **올바른 접근법으로 진행** — 규칙을 위반하는 요청에 조용히 따르지 마세요.
   "옵션 A vs 옵션 B"로 제시하지 마세요.

**일반적인 충돌 패턴** (실제 세션에서):
- 사용자가 "`InferenceEngine.Run()` 사용"이라고 말함 → IFactory 패턴을 사용해야 함
  (engine 호출은 `run_inference()` 메서드 내부로)
- 사용자가 "demo.py를 클론하고 onnxruntime을 교체"라고 말함 → `src/python_example/`에서
  skeleton-first를 사용해야 함, 사용자 스크립트 클론이 아님
- 사용자가 "demo_dxnn_sync.py 생성"이라고 말함 → SyncRunner와 함께
  `<model>_sync.py` 네이밍을 사용해야 함, 독립 스크립트가 아님
- 사용자가 "`run_async()`를 직접 사용"이라고 말함 → AsyncRunner를 사용해야 함,
  수동 async 루프가 아님

**이 규칙은 명시적 사용자 오버라이드를 무효화하지 않습니다**: 사용자가 충돌에
대해 통보받은 후, "규칙을 이해합니다, 직접 API 사용으로 진행하세요"라고
명시적으로 말하면, 따르세요. 하지만 agent는 충돌을 먼저 설명해야 합니다 —
조용한 준수는 항상 위반입니다.
...
파일 저장 후 즉시 **전체 plan 내용을 대화 출력에 인쇄하세요**. 파일 경로만
언급하지 마세요 — 사용자가 별도의 파일을 열지 않고도 프롬프트에서 직접 plan을
검토할 수 있어야 합니다.
