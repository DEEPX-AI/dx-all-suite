...
코드를 생성하거나, 파일을 만들거나, 서브 agent로 라우팅하기 전에, 다음 환경 확인이
반드시 통과해야 합니다. 이 게이트는 스위트 레벨에서 적용됩니다 — brainstorming이
spec과 plan을 생성했더라도, 이 확인은 구현 전에 반드시 실행되어야 합니다.
...
1. **Skeleton-first 개발** — 코드를 작성하기 전에 `dx-runtime/dx_app/.deepx/skills/dx-build-python-app.md`
...
2. **IFactory 패턴은 MANDATORY** — 모든 inference 앱은 IFactory 5-method 패턴
   (create, get_input_params, run_inference, post_processing, release)을 사용해야 합니다.
