...
| Skill | Description |
|-------|-------------|
| `/dx-validate-all` | Full validation across all 3 levels |
| `/dx-brainstorm-and-plan` | Process: collaborative design session before any work |
| `/dx-tdd` | Process: test-driven development — validate incrementally |
...
- **Correct**: `dx-runtime/dx_app/dx-agentic-dev/20260413-093000_plantseg_inference/demo_dxnn_sync.py`
...
7. **Pre-DONE mandatory deliverable check**: Before outputting DONE, run the
   self-verification check below. If any mandatory file is missing, create it
   before outputting DONE. **NEVER skip this step.**
   ```bash
   ...
   final report or output DONE with missing artifacts.
