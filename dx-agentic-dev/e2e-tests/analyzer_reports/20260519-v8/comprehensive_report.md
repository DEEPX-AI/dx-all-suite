# DEEPX Agentic Development — 종합 보고서 (Comprehensive Report)

> 생성 시각: 2026-05-19T11:16:34
> 이 보고서는 analysis.md(정량) + insights.md(정성) + runnability 요약을 통합한 종합본입니다.
> Raw 데이터는 마지막 §참고 섹션의 링크로 제공됩니다.
...
5개 AI 코딩 도구(Claude Code, Copilot CLI, Cursor CLI, OpenCode, Codex CLI)를 사용하여 NPU(Neural Processing Unit) 추론 앱을 자동 생성하는 "Agentic Development" 워크플로우의 실용성과 도구 간 성능 차이를 정량적으로 비교 평가한다.
...
# Executive Summary — 도구별 종합 순위

> **602** sessions = **5** tools × **21** rounds × **6** scenarios

## 종합 순위 (Overall Score 기준)

| Rank | Tool | Overall | Compliance | Quality | σ(Overall) | Sessions | Avg Duration |
|:----:|------|--------:|-----------:|--------:|-----------:|--------:|-------------:|
| 🥇 | **claude-code** | **77.6** | 93.2% | 98.2 | ±10.6 | 99/120 | 8m 53s |
| 🥈 | **cursor-cli** | **75.4** | 92.6% | 99.7 | ±10.8 | 90/120 | 12m 49s |
| 🥉 | **opencode-cli** | **74.0** | 92.4% | 95.9 | ±10.1 | 119/120 | 12m 1s |
| 4 | **codex-cli** | **73.5** | 93.0% | 97.8 | ±10.6 | 120 | 12m 12s |
| 5 | **copilot-cli** | **72.6** | 89.4% | 98.2 | ±10.1 | 122 | 13m 2s |

## 가성 결함 제외 (False Alarm Exclusion)

환경 문제(API rate limit, TLS error, CLI crash)로 인한 완전 실패 세션 **52건**이 점수 평균 산정 모수에서 제외되었습니다.
이 세션들은 도구 능력이 아닌 인프라 문제를 반영하므로 가성 결함으로 분류됩니다.

| 도구 | 제외 세션 수 | 유효 세션 수 | 제외 비율 |
|------|----------:|----------:|--------:|
| claude-code | 21 | 99/120 | 17.5% |
| cursor-cli | 30 | 90/120 | 25.0% |
| opencode-cli | 1 | 119/120 | 0.8% |

> 판정 기준: output_dirs 없음 + START sentinel 없음 + duration < 5초 → CLI가 agent를 시작하지 못한 환경 장애

## 주요 발견 (Key Findings)

- **최고 종합 점수**: claude-code (77.6)
- **최저 종합 점수**: copilot-cli (72.6)
- **점수 차이**: 5.0점 (6.9% 격차)
- **최고 Compliance**: claude-code (93.2%)
- **최고 Quality**: cursor-cli (99.7)
- **최단 평균 실행시간**: claude-code (8m 53s)

> 📊 **인터랙티브 차트**: [`dashboard.html`](./dashboard.html) 에서 시각적 비교 그래프를 확인할 수 있습니다.


---

# Part 1: 정량 분석 (analysis.md)
...
> insights.md가 아직 생성되지 않았습니다.
> 프롬프트: `/data/home/dhyang/github/dx-all-suite-full-e2e/dx-agentic-dev/e2e-tests/analyzer_reports/20260519-v8/insights_prompt.md`
> 수동 실행: `python3 insights.py --mode insights --report-dir <dir> --cli <copilot|claude>`

---

# Part 3: End-User Runnability — 요약

> 평가된 세션: **501** (raw: [`runnability_report.md`](./runnability_report.md))

## 3.1 분포 — Tool별

| Tool | PASS | PARTIAL | FAIL | Total | Runn % |
|------|----:|-------:|----:|-----:|------:|
| **claude-code** | 57 | 33 | 1 | 91 | 80.8 |
| **codex-cli** | 35 | 73 | 1 | 109 | 65.6 |
| **copilot-cli** | 34 | 70 | 6 | 110 | 62.7 |
| **cursor-cli** | 33 | 44 | 4 | 81 | 67.9 |
| **opencode-cli** | 32 | 77 | 0 | 109 | 64.7 |

## 3.2 분포 — Scenario별

| Scenario | PASS | PARTIAL | FAIL | Total | Runn % |
|----------|----:|-------:|----:|-----:|------:|
| compiler | 41 | 46 | 4 | 91 | 70.3 |
| dx_app | 40 | 52 | 1 | 93 | 71.0 |
| dx_stream | 33 | 56 | 1 | 90 | 67.8 |
| dx_stream_cascaded | 40 | 49 | 1 | 90 | 71.7 |
| runtime | 20 | 66 | 4 | 90 | 58.9 |
| suite | 17 | 28 | 1 | 46 | 67.4 |
...
- **cursor-cli R3 runtime** — Despite clear instructions and mostly complete setup, the session is not runnable as-is due to missing dx_com in the Python environment, blocking compilation
- **cursor-cli R4 dx_stream** — Without a README, session log, or clear run script, a typical end-user cannot reliably install, run, or verify this artifact
- **codex-cli R6 dx_app** — 필수 아티팩트가 모두 누락되어 있어, 사용자는 실행 또는 검증을 전혀 진행할 수 없습니다
- **claude-code R6 dx_stream_cascaded** — 필수 실행 및 안내 파일이 모두 누락되어, 사용자가 세션 산출물을 실행하거나 검증할 수 없습니다
- **copilot-cli R7 compiler** — 핵심 실행 및 설치 안내 파일이 모두 누락되어, 사용자가 세션 산출물을 실행하거나 검증할 수 없습니다
- **copilot-cli R10 runtime** — Without a README or session.log, a typical user cannot reliably install, run, or verify this artifact
- **copilot-cli R10 suite** — Without a README or session.log, a typical user cannot confidently install, run, or verify this artifact
- **copilot-cli R12 compiler** — README.md 부재와 컴파일 성공 확인 불가로 인해 일반 사용자가 이 세션을 독립적으로 설치·실행·검증하는 것은 현재 상태에서 불가능하다
- **copilot-cli R15 compiler** — README.md 전체 누락, 컴파일 완료 미확인, verify.py 부재로 인해 전형적인 사용자가 이 아티팩트를 독립적으로 설치·실행·검증하는 것은 사실상 불가능하다
- **cursor-cli R18 runtime** — `run.sh` 미존재, 컴파일·검증 실행 증거 부재로 end-user가 이 세션 결과물을 독립적으로 재현하는 것은 불가능하다
- **cursor-cli R19 compiler** — 모든 필수 아티팩트가 누락되어 있어 end-user가 이 세션의 결과물을 설치하거나 실행하는 것이 **불가능**하다
- **copilot-cli R20 runtime** — 필수 아티팩트(`setup.sh`, `run.sh`, `session.log`)가 모두 누락되어 사용자가 환경 구성부터 실행·검증까지의 어떤 단계도 가이드 없이 수행할 수 없으며, 이 세션은 **최소 납품 기준을 충족하지 못함**

## 3.5 PARTIAL 공통 패턴 (297건 중 키워드 빈도)

| 패턴 | 등장 세션 수 |
|------|-----------:|
| README 안내 부족 | 279 |
| setup.sh 환경 구성 불완전 | 229 |
| session.log 부재/부족 | 221 |
| venv 처리 부재 | 122 |
| dx_engine bridging 누락 | 84 |
| verify.py 미제공 | 64 |
| 경로 하드코딩 | 58 |
| SUITE_ROOT 미사용 (상대경로 의존) | 49 |
...
- 정성 평가 (세션별 raw): [`runnability_report.md`](./runnability_report.md)
- 정량 분석 (세션 단위 raw JSON): [`analysis.json`](./analysis.json)
- 세션 행 단위 CSV: [`per_session.csv`](./per_session.csv)
- 분석 단계별 .md: [`analysis.md`](./analysis.md) · [`insights.md`](./insights.md)
- 인터랙티브 대시보드: [`dashboard.html`](./dashboard.html)
