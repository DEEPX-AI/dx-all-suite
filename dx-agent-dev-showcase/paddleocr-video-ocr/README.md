...
Short and goal-only — it names no toolset path, file, or branch; the skill + KB routing
supply those (and the decision to generate a standalone app over the fork's pipeline):
...
window). Save one annotated sample frame as sample_detect.jpg. Provide setup.sh, run.sh,
